...
function SchoolsCarousel() {
  ...
  if (escuelas.length === 0) {
    return <div className="text-center py-8">Cargando escuelas...</div>;
  }
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 space-y-5 max-w-7xl w-full">
        <h2 className="text-4xl font-extrabold text-center capitalize mb-8 dark:text-white">Escuelas destacadas</h2>
        ...
        >
          {escuelas.map((escuela) => (
            <SwiperSlide key={escuela.escuela_id} className="bg-center bg-cover w-[300px] sm:w-[350px] md:w-[400px] lg:w-[450px] dark:drop-shadow-[0_0_8px_#FBBF24] drop-shadow-[0_0_8px_#FBBF24]">
              <div className="card bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105 dark:bg-black">
                <figure className="relative">
                  <img 
                    src={escuela.escuela_imagen_url} 
                ...
                <div className="card-body p-4">
                  <h2 className="card-title text-xl font-semibold mb-2 dark:text-white">{escuela.escuela_nombre}</h2>
                  <p className="text-lg mb-2 dark:text-gray-300 line-clamp-2">
                    {escuela.escuela_descripcion}
                  </p>
                  <p className="text-sm mb-2 dark:text-gray-400">Dirección: {escuela.escuela_direccion}</p>
                  <div className="card-actions justify-end pt-2 pb-2">
                    <a 
                      href={`../APP/Views/Crud/escuela_detalle.php?id=${escuela.escuela_id}`} 
